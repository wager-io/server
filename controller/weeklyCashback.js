...
const weeklyCashback = (async (req, res)=>{
 let user_id = req.id

 // console.log(user_id)

 try {
  const query = `SELECT * FROM profiles WHERE vip_level > 20 AND weekly_wagered >= 1000 AND user_id = "${user_id}"`;
      connection.query(query, async function (error, response) {
       if(response && (response.length !== 0)){
        // console.log(response[0])
         ...
         const amountToReceive = +response[0].weekly_wagered * 0.01 * percentage;
         ...
         await helper.clearWeeklyWagered(response[0].user_id)
