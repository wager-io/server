...
const {createWGF, createEth, createbtc, 
    createwagerToken  } = require("../wallet_transaction/index")
    ...
    try {
        ...
        const membersDataFromProfile = await Promise.all(
            members.map(async (user) => {
                ...
                const eth_balance = await ETHWallet.findOne({ user_id: user.user_id })
                const btc_balance = await BTCWallet.findOne({ user_id: user.user_id })
                const wgd_balance = await WGDWallet.findOne({ user_id: user.user_id })
                ...
                //Sum in USD
                const totalBalance = (eth_balance.balance + btc_balance.balance + conversion(wgd_balance.balance))
                return {
                    ...user._doc,
                    profile,
                    userFirstAndLastDeposit,
